...
const pool = mysql.createPool({
    ...
    password: process.env.DB_PASSWORD || 'your_password', // Mật khẩu
    database: process.env.DB_NAME || 'your_database', // Tên cơ sở dữ liệu
...
const handleRegister = async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(400).render('auth/errorRegister', { error: 'Email và mật khẩu là bắt buộc' });
    }
    ...
    try {
        ...
        if (existingUsers.length > 0) { // Nếu email đã tồn tại
            return res.status(400).render('auth/errorRegister', { error: 'Email đã tồn tại' });
        }
